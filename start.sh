...
writeMapping() {
    ...
    if [ ! -z "$BLACKLIST_SRC" ]; then
        echo "    acl blacklist_src src -f blacklist.txt" >> haproxy.cfg
        echo "    tcp-request connection reject if blacklist_src" >> haproxy.cfg
    fi
